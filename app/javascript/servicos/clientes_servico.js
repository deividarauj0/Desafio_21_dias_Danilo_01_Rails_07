...
export class ClienteServico {
    static async todos(paginaCorrente =1){
        const clientesData = await (await fetch(`http://[::1]:3000/clientes.json?pagina=${paginaCorrente}`)).json();
    ...
    static async delete(id, authenticity_token) {
        ...
        const url = `http://[::1]:3000/clientes/${id}.json`;
    ...
    static async salvar(cliente, authenticity_token) {
        ...
        if(this.cliente.id < 1)
            ...
            url = `http://[::1]:3000/clientes/${cliente.id}.json`;
        }
