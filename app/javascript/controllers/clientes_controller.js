...
export default class extends Controller {
  ...
  async loadClientes(event) {
    ...
    const clientes = await (await fetch(`http://[::1]:3000/clientes.json?pagina=${this.paginaCorrente}`)).json();
  ...
  async salvar(event) {
    ...
    if(this.cliente.id < 1)
      ...
      url = `http://[::1]:3000/clientes/${this.cliente.id}.json`;
    }
  ...
  async excluir(event) {
    ...
    if(confirm("Deseja realmente excluir?")) {
      ...
      const url = `http://[::1]:3000/clientes/${cliente.id}.json`;
